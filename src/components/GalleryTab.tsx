        ...
        {lightboxPhoto && (
            <div className="fixed inset-0 z-[300] bg-black/95 backdrop-blur-md flex flex-col animate-in fade-in duration-300">
                <div className="absolute top-4 right-4 z-50 flex gap-2">
                    <button onClick={() => setIsEditingPhoto(!isEditingPhoto)} className={`p-3 rounded-full transition-colors ${isEditingPhoto ? 'bg-blue-600 text-white' : 'bg-white/10 hover:bg-white/20 text-white'}`}>
                        <Edit2 size={20}/>
                    </button>
                    <button onClick={() => handleDelete(lightboxPhoto.id)} className="p-3 bg-white/10 hover:bg-red-500/20 text-white/70 hover:text-red-400 rounded-full transition-colors"><Trash2 size={20}/></button>
                    <button onClick={() => setLightboxPhoto(null)} className="p-3 bg-white/10 hover:bg-white/20 text-white rounded-full transition-colors"><X size={24}/></button>
                </div>
