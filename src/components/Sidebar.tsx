            ...
            {menuItems.map((item) => (
              <li key={item.id}>
                <button
                  ...
                  className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} px-3 py-2 rounded-lg text-xs font-bold transition-all duration-200 group relative ${
                    activeTab === item.id
                ...
                >
                  {item.icon}
                  {!isCollapsed && <span className="uppercase tracking-tight">{item.label}</span>}
                </button>
              </li>
            ))}
          </ul>
