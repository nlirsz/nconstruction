...
export const ProjectSettings: React.FC<ProjectSettingsProps> = ({ project, onUpdateProject, organizations, currentUser }) => {
    const [activeTab, setActiveTab] = useState<'general' | 'team' | 'structure' | 'phases' | 'clients'>('general');
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <div className="flex gap-2 mb-6 border-b border-slate-200 overflow-x-auto">
                ...
                <button type="button" onClick={() => setActiveTab('clients')} className={`px-4 py-2 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${activeTab === 'clients' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}><ShieldCheck size={16} /> Clientes & Arq.</button>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-slate-800 bg-white"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1.5">Formato de Exibição (Portal Cliente)</label>
                                        <select
                                            value={formData.deliveryFormat || 'month'}
                                            onChange={e => setFormData({ ...formData, deliveryFormat: e.target.value as 'month' | 'semester' })}
                                            className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-slate-800 bg-white text-sm"
                                        >
                                            <option value="month">Mês / Ano (Ex: Jan de 2028)</option>
                                            <option value="semester">Semestre / Ano (Ex: 1º Semestre de 2028)</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                ...
                {activeTab === 'clients' && (
                    <div className="pb-20">
                        <ClientManagement project={project} levels={levels} />
                    </div>
                )}
