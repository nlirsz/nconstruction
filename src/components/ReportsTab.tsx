...
export const ReportsTab: React.FC<ReportsTabProps> = ({ project, userProfile, organization }) => {
    ...
    const [monthPhotos, setMonthPhotos] = useState<any[]>([]);
    ...
    const fetchData = async () => {
        ...
        if (photos) setMonthPhotos(photos);
    ...
    const chartData = useMemo(() => {
        ...
        // Simplificação: pega o progresso reportado nos logs para construir curva
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            let avgGain = 0;
            if (dayLogs.length > 0) {
                avgGain = dayLogs.reduce((acc, curr) => acc + (curr.gain || 0), 0) / PHASES.length;
            }

            const prevDayProgress = data.length > 0 ? data[data.length - 1].progress : 0;
            data.push({
                day: i,
                progress: Math.min(100, prevDayProgress + avgGain * 5) // Multiplicador para visualização s-curve
            });
        }
        return data;
    }, [logs, selectedMonth, selectedYear]);
            ...
            <div className="bg-slate-200 p-2 md:p-10 rounded-3xl overflow-hidden">
                <div id="report-content" className="bg-white shadow-2xl mx-auto max-w-[800px] min-h-[800px] md:min-h-[1000px] p-[1.5cm] md:p-[2cm] text-slate-800" style={{ fontFamily: 'Arial, sans-serif', lineHeight: '1.5', color: '#1e293b' }}>
                    ...
                    <div style={{ marginBottom: '40px', color: '#1e293b' }}>
                        ...
                        {/* Gráfico de Evolução (Híbrido) */}
                        <div style={{ height: '200px', width: '100%', marginBottom: '20px', background: '#f8fafc', borderRadius: '12px', padding: '10px' }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={chartData}>
                                    ...
                                    <Tooltip />
                                    <Area type="monotone" dataKey="progress" stroke="#3b82f6" fill="#dbeafe" strokeWidth={2} />
                                </AreaChart>
                            </ResponsiveContainer>
                            <p style={{ textAlign: 'center', fontSize: '7pt', color: '#64748b', marginTop: '5px' }}>Curva de tendência de avanço acumulado no período</p>
                        </div>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '9pt', border: '1px solid #e2e8f0', color: '#334155' }}>
                            <thead>
                                <tr style={{ background: '#1e293b', color: 'white' }}>
                                    ...
                                    <th style={{ padding: '8px', textAlign: 'center' }}>Progresso Total (%)</th>
                                    <th style={{ padding: '8px', textAlign: 'center' }}>Ganho Mensal</th>
                                </tr>
                            </thead>
                            <tbody>
                                {PHASES.map(p => {
                                    ...
                                    return (
                                        <tr key={p.id} style={{ borderBottom: '1px solid #e2e8f0', color: '#334155' }}>
                                            ...
                                            <td style={{ padding: '6px 8px', textAlign: 'center' }}>
                                                <div style={{ width: '100%', height: '8px', background: '#f1f5f9', borderRadius: '4px', overflow: 'hidden', marginBottom: '4px' }}>
                                                    <div style={{ width: `${current}%`, height: '100%', background: '#3b82f6' }}></div>
                                                </div>
                                                {current}%
                                            </td>
                    ...
                    <div style={{ marginBottom: '40px', color: '#1e293b' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px' }}>
                            {monthPhotos.length > 0 ? monthPhotos.slice(0, 6).map((photo, i) => (
                                <div key={i} style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '10px', background: '#fff', pageBreakInside: 'avoid', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                                    <img src={photo.url} style={{ width: '100%', height: '180px', objectFit: 'cover', borderRadius: '8px', marginBottom: '8px' }} alt="Foto" />
                                    <p style={{ fontSize: '8pt', fontWeight: 'bold', margin: '0', color: '#1e293b' }}>{photo.description || 'Sem descrição'}</p>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                                        <span style={{ fontSize: '7pt', color: '#94a3b8' }}>{photo.category || 'Geral'}</span>
                                        <span style={{ fontSize: '7pt', color: '#94a3b8' }}>{new Date(photo.created_at).toLocaleDateString('pt-BR')}</span>
                                    </div>
                                </div>
                            )) : (
                                <div style={{ gridColumn: 'span 2', textAlign: 'center', padding: '40px', border: '2px dashed #e2e8f0', borderRadius: '12px', color: '#94a3b8', fontSize: '10pt' }}>
