        ...
        const fetchSummaries = async () => {
            ...
            try {
                ...
                const { data: taskData } = await supabase
                    .from('tasks')
                    .select('project_id, end, progress')
                    .in('project_id', projectIds)
                    .neq('status', 'Concluído');

                const timelineAlertCounts: Record<string, number> = {};
                const today = new Date();
                taskData?.forEach((t: any) => {
                    if (t.progress < 100 && new Date(t.end) < today) {
                        timelineAlertCounts[t.project_id] = (timelineAlertCounts[t.project_id] || 0) + 1;
                    }
                });
                ...
                projectIds.forEach(pid => {
                    summaries[pid] = {
                        ...
                        timelineAlerts: timelineAlertCounts[pid] || 0
                    };
                });
                ...
                <div className="w-full flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center justify-between w-full md:w-auto">
                        <div className="flex items-center gap-3">
                            <img src={APP_LOGO_URL} alt="NC Gestão" className="h-6 md:h-8" />
                            <div className="h-4 w-px bg-slate-200 hidden md:block"></div>
                            <div>
                                <h1 className="text-sm md:text-base font-black text-slate-800 tracking-tight leading-none uppercase text-center md:text-left">Gestor de Obras</h1>
                                <p className="text-[8px] md:text-[9px] font-black text-slate-400 border-t border-slate-50 pt-0.5 uppercase tracking-widest hidden md:block mt-0.5">Visão Geral do Portfólio</p>
                            </div>
                        </div>

                        <div className="flex items-center gap-2 md:hidden">
                            <button onClick={onOpenProfile} className="w-7 h-7 bg-slate-50 rounded-full overflow-hidden border border-slate-200 flex">
                                {userProfile?.avatar_url ? <img src={userProfile.avatar_url} className="w-full h-full object-cover" /> : <User size={14} className="m-auto" />}
                            </button>
                            <button onClick={handleOpenAdd} className="bg-blue-600 text-white p-1.5 rounded-lg shadow-sm"><Plus size={16} /></button>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2 w-full md:w-auto items-center">
                        <div className="relative flex-1 md:w-64">
                            ...
                            <input type="text" placeholder="Buscar projeto..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 rounded-lg border-none text-[10px] md:text-xs text-slate-800 bg-slate-50 outline-none focus:ring-2 focus:ring-blue-500/10 font-bold" />
                        </div>
                        <button onClick={handleOpenAdd} className="hidden md:flex bg-blue-600 text-white px-4 py-2 rounded-lg font-black text-xs items-center gap-2 transition-all hover:bg-blue-700 shadow-md shadow-blue-100 uppercase tracking-widest"><Plus size={14} /> Nova Obra</button>
                        <button onClick={onCreateOrganization} className="p-2 md:px-3 md:py-1.5 text-[9px] font-black uppercase text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors border border-blue-100 shrink-0" title="Nova Empresa">
                                        ...
                                        <div className="relative h-44 overflow-hidden shrink-0 bg-slate-100">
                                            ...
                                            <div className="absolute bottom-3 left-4 right-4">
                                                <div className="flex items-center gap-2 mb-1.5 ">
                                                    <span className={`inline-block px-1.5 py-0.5 text-[8px] font-black rounded uppercase tracking-widest ${displayStatus === 'green' ? 'bg-emerald-500 text-white' : displayStatus === 'yellow' ? 'bg-amber-500 text-white' : 'bg-red-500 text-white'}`}>{displayStatus === 'green' ? 'Estável' : 'Alerta'}</span>
                                                    {projectSummaries[project.id]?.timelineAlerts > 0 && (
                                                        <span className="bg-red-600 text-white px-1.5 py-0.5 rounded text-[8px] font-black uppercase flex items-center gap-1 animate-pulse shadow-lg ring-1 ring-white/50">
                                                            <AlertTriangle size={8} /> Atrasos
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        <div key={`dash-${project.id}`} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col transition-all">
                                            <div className="flex flex-col md:flex-row">
                                                ...
                                                <div className="flex-1 p-4 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                                                    <div className="flex items-center gap-3">
                                                        <Clock size={16} className={summary?.timelineAlerts ? "text-red-500 animate-pulse" : "text-slate-300"} />
                                                        <div className="flex flex-col">
                                                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Cronograma</span>
                                                            <span className={`font-black text-xs ${summary?.timelineAlerts ? 'text-red-600' : 'text-slate-700'}`}>
                                                                {summary?.timelineAlerts ? `${summary.timelineAlerts} Pendências` : 'Em dia'}
                                                            </span>
                                                        </div>
                                                    </div>
                                                    <div className="flex items-center gap-3">
