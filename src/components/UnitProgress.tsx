...
export const UnitProgress: React.FC<UnitProgressProps> = ({ project, onNavigateToExecution }) => {
  ...
  const [unitDetail, setUnitDetail] = useState<{ floorIdx: number, unitIdx: number } | null>(null);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (levels && levels.length > 0) {
                ...
                setData(finalData);
                if (finalData.length > 0) setExpandedFloors([finalData[0].floor]);
            }
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {globalStats.map(stat => (
                  <div key={stat.id} className="bg-slate-50 p-3 rounded-lg border border-slate-100 flex flex-col items-center text-center">
                      <div className={`p-1 rounded text-white ${getPhaseColor(stat.color)} mb-2 shadow-sm`}>{getPhaseIcon(stat.icon, 10)}</div>
      ...
      {viewMode === 'matrix' ? (
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col flex-1 overflow-hidden">
              ...
              <div className="overflow-auto flex-1 custom-scrollbar relative">
                  <div className="min-w-max">
                      ...
                      <div className="p-0">
                          {[...data].reverse().map((floor, reverseIdx) => {
                              const originalIdx = data.length - 1 - reverseIdx;
                              ...
                              <div key={floor.floor} className="border-b border-slate-100 last:border-0">
                                  ...
                                  <div className={`grid gap-0 transition-colors sticky left-0 z-[35] w-full border-b border-slate-50 ${isExpanded ? 'bg-blue-50/10' : 'bg-white hover:bg-blue-50/5'}`} style={{ gridTemplateColumns }}>
                                      ...
                                      {!isExpanded && projectPhases.map((p) => {
                                          const avg = floorStats[originalIdx].averages[p.id];
                                          return (
                                              <div key={`avg-${p.id}`} className="h-full border-r border-slate-50 last:border-0 flex items-center justify-center p-1">
                                      ...
                                      <div className="bg-white">
                                          {floor.units.map((unit: any, uIdx: number) => (
                                              <div key={unit.id} className="grid gap-0 group/unit bg-white hover:bg-slate-50 border-b border-slate-50 last:border-0" style={{ gridTemplateColumns }}>
                                                  <div onClick={() => { setUnitDetail({ floorIdx: originalIdx, unitIdx: uIdx }); }} className="sticky left-0 z-[30] bg-white border-r border-slate-200 shadow-sm px-4 py-1.5 text-[8px] font-bold text-slate-700 flex items-center gap-2 cursor-pointer transition-colors group-hover/unit:bg-blue-50">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col flex-1 overflow-hidden p-4 relative min-h-[500px]">
              ...
              <div className="flex-1 bg-slate-50/20 flex items-center justify-center overflow-hidden">
                  <BuildingVisualizer project={project} data={data} onUnitClick={(e, f, u) => setUnitDetail({ floorIdx: f, unitIdx: u })} />
              </div>
          </div>
      )}
