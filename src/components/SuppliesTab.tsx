...
export const SuppliesTab: React.FC<SuppliesTabProps> = ({ project, currentUser, userProfile }) => {
    ...
    const fetchOrders = async () => {
        ...
        if (data && !error) {
            const fbData = data.map((order: any) => ({
                ...order,
    ...
    const handleUpdateOrder = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedOrder || !newOrderTitle.trim()) return;
        ...
        <div className="pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {isCreateOpen && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[300] flex items-center justify-center p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg border border-slate-100 flex flex-col max-h-[90vh] overflow-hidden">
                        <div className="p-4 border-b border-slate-50 flex justify-between items-center bg-white shrink-0">
                            <h3 className="font-semibold text-slate-800 text-sm">{isEditing ? 'Editar Solicitação' : 'Nova Solicitação'}</h3>
                            <button onClick={() => { setIsCreateOpen(false); resetForm(); }} className="text-slate-400 hover:bg-slate-50 p-1.5 rounded-lg transition-colors"><X size={18} /></button>
                        </div>

                        <form id="supply-form" onSubmit={isEditing ? handleUpdateOrder : handleCreateOrder} className="flex-1 overflow-y-auto p-5 space-y-5 custom-scrollbar">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Título do Pedido</label>
                                    <input
                                        required
                                        autoFocus
                                        type="text"
                                        placeholder="Cimento, Tintas, etc..."
                                        value={newOrderTitle}
                                        onChange={e => setNewOrderTitle(e.target.value)}
                                        className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:border-blue-500 outline-none text-sm text-slate-700 bg-slate-50/50 transition-all font-medium"
                                    />
                                </div>

                                <div>
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Prioridade</label>
                                    <div className="flex gap-2">
                                        {(['low', 'medium', 'high'] as SupplyPriority[]).map((p) => (
                                            <button
                                                key={p}
                                                type="button"
                                                onClick={() => setNewOrderPriority(p)}
                                                className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg border transition-all uppercase tracking-tight ${newOrderPriority === p ? 'bg-slate-900 text-white border-slate-900 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}
                                            >
                                ...
                                <div className="pt-2">
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3 flex justify-between items-center">
                                        Itens e Materiais
                                        <span className="text-slate-300 font-normal">({newItems.length})</span>
                                    </label>
                                    <div className="space-y-2">
                                        {newItems.map((item, idx) => (
                                            <div key={item.id} className="flex gap-2 animate-in slide-in-from-left-2 duration-200">
                                                <input type="text" placeholder="Nome do item" value={item.name} onChange={e => { const list = [...newItems]; list[idx].name = e.target.value; setNewItems(list); }} className="flex-1 px-3 py-1.5 border border-slate-200 rounded-lg text-xs text-slate-700 bg-white focus:border-blue-400 outline-none font-medium" />
                                                <input type="number" placeholder="0" value={item.quantity} onChange={e => { const list = [...newItems]; list[idx].quantity = Number(e.target.value); setNewItems(list); }} className="w-14 px-2 py-1.5 border border-slate-200 rounded-lg text-xs text-center text-slate-700 bg-white focus:border-blue-400 outline-none font-bold" />
                                                <input type="text" placeholder="un" value={item.unit} onChange={e => { const list = [...newItems]; list[idx].unit = e.target.value; setNewItems(list); }} className="w-12 px-2 py-1.5 border border-slate-200 rounded-lg text-xs text-center text-slate-500 bg-slate-50/50 focus:border-blue-400 outline-none font-bold" />
                                                <button type="button" onClick={() => setNewItems(newItems.filter((_, i) => i !== idx))} className="p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={14} /></button>
                                            </div>
                                        ))}
                                    </div>
                                    <button type="button" onClick={() => setNewItems([...newItems, { id: Date.now().toString(), name: '', quantity: 1, unit: 'un', checked: false }])} className="mt-4 w-full py-2 border-2 border-dashed border-slate-100 rounded-xl text-[10px] font-bold text-blue-600 flex items-center justify-center gap-1.5 hover:bg-blue-50/50 hover:border-blue-100 transition-all uppercase tracking-widest"><Plus size={14} /> Adicionar Item</button>
                                </div>
                            </div>
                        </form>

                        <div className="p-4 border-t border-slate-50 bg-slate-50/50 rounded-b-xl flex justify-end gap-2 shrink-0">
                            <button onClick={() => { setIsCreateOpen(false); resetForm(); }} className="px-4 py-2 text-slate-400 font-bold text-[10px] uppercase tracking-widest hover:text-slate-600">Cancelar</button>
                            <button form="supply-form" type="submit" disabled={submitLoading} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg text-xs shadow-md hover:bg-blue-700 disabled:opacity-50 transition-all uppercase tracking-widest">
                                {submitLoading ? 'Processando...' : (isEditing ? 'Salvar Alterações' : 'Enviar Pedido')}
                            </button>
                        </div>
                    </div>
                </div>
