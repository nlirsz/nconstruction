...
import { 
    ...
    BarChart3, GripVertical, Loader2, 
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in duration-500">
                        ...
                        <tr key={task.id} className="hover:bg-blue-50/30 transition-colors group cursor-pointer" onClick={() => handleOpenEdit(task)}>
                            ...
                            <td className="px-4 py-2">
                                <div className="flex flex-col">
                                    <span className={`font-black text-[10px] uppercase leading-tight ${task.status === TaskStatus.COMPLETED ? 'text-slate-400' : 'text-slate-800'}`}>{task.name}</span>
                            ...
                            <td className="px-4 py-2 text-center">
                                <span className={`text-[7px] font-black uppercase px-1.5 py-0.5 rounded-md border ${task.status === TaskStatus.COMPLETED ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : task.progress > 0 ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
                                    {task.status}
                                </span>
                            </td>
                            <td className="px-4 py-2">
                                <div className="flex items-center gap-2">
                                    <div className="flex-1 h-1 bg-slate-100 rounded-full overflow-hidden">
                                        <div className={`h-full transition-all ${task.progress === 100 ? 'bg-emerald-500' : 'bg-blue-500'}`} style={{ width: `${task.progress}%` }}></div>
                                    </div>
                        ...
                        return (
                            <div key={task.id} className="flex border-b border-slate-50 group hover:bg-slate-50/50">
                                <div className="w-40 shrink-0 border-r border-slate-100 px-3 py-2 flex flex-col justify-center overflow-hidden">
                                    <span className="text-[8px] font-black text-slate-800 uppercase truncate">{task.name}</span>
                                    <span className="text-[6px] font-bold text-slate-400 uppercase truncate">{task.customId || 'Geral'}</span>
                                </div>
                                <div className="flex-1 relative h-10 flex items-center">
                                    <div 
                                        onClick={() => handleOpenEdit(task)}
                                        className={`absolute h-5 rounded shadow-sm cursor-pointer transition-transform hover:scale-[1.02] flex items-center px-1.5 overflow-hidden border border-white/20 ${task.status === TaskStatus.COMPLETED ? 'bg-emerald-500' : 'bg-blue-600'}`}
                                    ...
                                    >
                                        ...
                                        <span className="text-[6px] font-black text-white uppercase truncate relative z-10">{task.progress}%</span>
                                    </div>
                                </div>
                            </div>
                        );
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-in zoom-in duration-200 flex flex-col max-h-[85vh]">
                ...
                <form onSubmit={handleTaskSubmit} className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                    <div className="space-y-4">
                        <div>
                            <label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Título da Atividade</label>
                        ...
                        <div className="p-3 bg-blue-50/50 rounded-lg border border-blue-100 space-y-3">
                            <label className="block text-[8px] font-black text-blue-600 uppercase mb-1">Vínculos de Execução</label>
                            <div className="grid grid-cols-2 gap-3">
                                <select 
                                    ...
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-[9px] bg-white font-bold"
                                >
                                ...
                                <select 
                                    ...
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-lg text-[9px] bg-white font-bold"
                                >
                                    <option value="">Fase Técnica</option>
